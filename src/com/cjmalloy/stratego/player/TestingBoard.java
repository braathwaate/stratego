...
assert p.getRank() != Rank.UNKNOWN : "Unknown cannot be known or suspected " + p.isKnown() + " " + p.isSuspectedRank();
	...
	protected int setProtector(Piece p, int to)
	{
        ...
        // TBD: not all unknowns are protectors: check flee rank
                ...
                if (!tp.isKnown()
                    || tprank.ordinal() < p.getRank().ordinal())
                    attackPiece = tp;
			}
		}
...
*/
		...
		// latter uses the apparent piece value.
                    ...
                    else if (isStrongExpendable(tp) && !fp.is(Piece.WEAK)
	...
	int apparentRisk(Piece fp, Rank fprank, boolean unknownScoutFarMove, Piece tp)
	{
        ...
        // worth attacking to reveal its rank.  So the AI must hold its
        // ground, even with the likely possibility that the Five
        // will lotto.  However, if the approaching piece is expendable,
