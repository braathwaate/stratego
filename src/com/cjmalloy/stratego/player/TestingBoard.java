...
@SuppressWarnings("unchecked")
	...
	private void genValueStealth()
	{
        ...
        // Note: (value(3) + value(4))/2 is not correct, because if the Threes
        // are gone from the board, value(3) becomes value(4).  If the Twos
        // and Fours are gone, then value(1) becomes value(3) and value(4),
        // which would make the stealth value of a Spy the same as the One
        // value, meaning the Spy would not attack the One.  By tying the
        // stealth value of the Spy to the Three value, the Spy often becomes
        // the most valuable piece after the One during endgames, which is
        // correct because the Spy becomes essential to winning these endgames.

            valueStealth[c][9] = values[c][3] * 2 / 3;
	...
	void attackLanes()
	{
        ...
        // is actually a piece, then it cannot pass
					...
					int ranksNeeded = 2;
					for (int r = oppRank; ranksNeeded > 0 && r > 1; r--)
...
assert p.getRank() != Rank.UNKNOWN : "Unknown cannot be known or suspected " + p.isKnown() + " " + p.isSuspectedRank();
	...
	protected int setProtector(int aito, Piece p, int to)
	{
        int color = p.getColor();
	...
	protected int setAdjacentProtector(int to, int i)
	{
        ...
        // xx xx b5 --
        // -- -- r? --
        // Unknown Red flees unknown Blue to the right.  Unknown Blue Five moves
	...
	protected void addPlanValue(Piece fp, int from, int to, boolean scoutFarMove)
	{
        ...
        assert (Math.abs(vm) <= DEST_PRIORITY_DEFEND_FLAG_STAY);
	...
	// A known Unknown (isKnown() true and either Rank of UNKNOWN
    // or isSuspectedRank() true).  The value of a suspected rank
	// is diminished in actualValue(), and it is also known,
    // so a further attack on
	...
	public void makeWinner(Piece p, Rank rankWon, boolean aiAttack)
	{
		...
		if (!isSuspected) {
			...
			if (newRank == Rank.NIL)
				newRank = Rank.UNKNOWN; // known Unknown
        }
...
*/
        ...
        if (fpcolor == Settings.topColor) {
            ...
            // remains safe if pursuing a two squares chase
            // or if an opponent piece attacked
            if (fp.isKnown()
                && fp.is(Piece.SAFE)
...
*/
		...
		// make an even exchange.
                    ...
                    if (isAggressive(tp, fp))
                        tpvalue = tpvalue * 9 / 10;
		...
		// make an even exchange.
                ...
                if (isAggressive(fp, tp))
                    fpvalue = fpvalue * 9 / 10;
...
*/
	...
	public boolean isEffectiveBluff(Piece fp, Piece tp)
	{
		...
		if (fp.isKnown()
			|| (isExpendable(tp) && !isFlagBombAtRisk(tp))
	...
	int apparentRisk(Piece fp, Rank fprank, boolean unknownScoutFarMove, Piece tp)
	{
		...
		if (unknownScoutFarMove) {
            ...
            // Note: if risk is high and a stealthy AI piece is subject to attack,
            // the AI will blunder because the horizon effect

            risk = Math.min(risk, weakRanks(fp.getColor())*5);
    ...
    // Could be a bomb
            ...
            if (tp.getRank() != Rank.BOMB
                && fprank.ordinal() <= 7)
                v += fpvalue/ (10 - fprank.ordinal());
            else
                v += fpvalue/ (20 - fprank.ordinal());
        }
    ...
    // pieces.
        ...
        if ((((fp.isKnown()
            ...
            || (tp.getRank() == Rank.UNKNOWN
                && fprank == Rank.FOUR))
            && isAggressive(fp, tp))
                v += values[Settings.topColor][Rank.FIVE.ordinal()];
    ...
    public boolean isAggressive(Piece fp, Piece tp)
    {
        ...
        return isForay(tp.getIndex())  // goal is to clear the foray line
            ...
            || dangerousUnknownRank < fprank.ordinal() // increase aggression
            || dangerousKnownRank < fprank.ordinal();   // increase aggression
    }
    ...
    // Return a result between 0 and v, depending on the value vm.
