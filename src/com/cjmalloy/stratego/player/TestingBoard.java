...
public class TestingBoard extends Board
{
	...
	private static final int VALUE_NINE = VALUE_SIX * 30/100;
...
@SuppressWarnings("unchecked")
					...
					for (int r = oppRank; ranksNeeded <= 3 && r > 1; r--) {
						if (rankAtLarge(c, r) != 0) {
							...
							if (!isStealthy(planAPiece[c][r-1], oppRank))
								genNeededPlanA(false, tmp2, c, r, DEST_PRIORITY_CHASE);
	...
	void setUnmovedValues()
	{
		...
		// piece and it is needed, break up structure.
			...
			if (isNeededRank(p)
                ...
                // If the piece is buried by other unmoved pieces,
                // try to unbury it by moving one of the blocking pieces.
                // This is necessary for bad setups, of which the AI
                // has many.   The AI often loses because it does not
                // have an unburied defender to protect the flag.
	...
	// Launch an attack on the flag bomb structure.
	//
...
assert p.getRank() != Rank.UNKNOWN : "Unknown cannot be known or suspected " + p.isKnown() + " " + p.isSuspectedRank();
		...
		// making the player Four invincible, because 1?x4 is positive).
				...
				genPlanA(rnd.nextInt(2) != 0, destTmp2, 1-p.getColor(), found, priority);
				genPlanB(rnd.nextInt(2) != 0, destTmp2, 1-p.getColor(), found, priority);
			}
				...
				if (isInvincible(1-p.getColor(), j))
					...
					if (chaser != null
						...
						if (chaser != null && !chaser.isKnown())
							genPlanA(rnd.nextInt(10) != 0, destTmp[GUARDED_OPEN], 1-p.getColor(), j, DEST_PRIORITY_CHASE);
						else
		...
		// an expendable, so if they are discovered, its a wash.
			...
			for ( int j = 5; j >= 4; j--) {
				if (isInvincible(1-p.getColor(), j))
					continue;
				int destTmp2[] = genDestTmpGuardedOpen(p.getColor(), i, Rank.toRank(j));
				genNeededPlanA(rnd.nextInt(2) != 0, destTmp2, 1-p.getColor(), j, DEST_PRIORITY_CHASE);
				genPlanB(rnd.nextInt(2) != 0, destTmp2, 1-p.getColor(), j, DEST_PRIORITY_CHASE);
			}
		//
		...
		// So this code is no longer needed and was removed.
			...
			if (!p.isWeak() || isForay(p.getIndex()))
		...
		// guessing.
				...
				if (hasFewWeakRanks(p.getColor(), 4)
					|| p.getIndex() <= 65) {
					genNeededPlanA(rnd.nextInt(10) != 0, destTmp2, 1-p.getColor(), j, priority);

				} else if (isInvincibleDefender(1-p.getColor(), j))
					genPlanA(rnd.nextInt(2) != 0, destTmp2, 1-p.getColor(), j, priority);
				// tbd: PlanB as well
				if (!isInvincible(p))
					chaseWithUnknown(p, destTmp[GUARDED_UNKNOWN]);
...
*/
	...
	public boolean chaseMove(int m)
	{
		...
		// Note: fp.setSafe() must be after addMoveValue()
		// because addMoveValue saves history

		if (isTwoSquaresChase()) {
			value++;

                // Risk increases as weak pieces are removed from the board
                // because the remaining pieces are stronger

                        if (!hasFewWeakRanks(1-fp.getColor(), 8))
                            fp.setSafe(true);
...
*/
		...
		// The opponent is a suspected rank, but it could
		// be some other rank, perhaps weaker.
		// The AI loses its piece value but gains riskOfWin().
		// So if the AI has the choice between certain loss
		// from a known rank and uncertain loss from
		// a suspected rank, it chooses uncertain loss.

						else {
							vm -= fpvalue;
                                                        if (!tp.isKnown())
                                                            vm += riskOfWin(fpvalue, fp, tp);
						}
						tp.setKnown(true);
						if (fprank.ordinal() < 9)
		...
		// so the remaining unmoved pieces are fair game?
                                        ...
                                        vm += Math.min(2*valueStealth[1-fpcolor][Rank.BOMB.ordinal()-1], stealthValue(1-fpcolor, unknownRank[1-fpcolor])) + riskOfWin(fpvalue, fp, tp) + unmovedValue[tp.getIndex()] - fpvalue;
	...
	public int unknownValue(int fpvalue, Piece fp, Piece tp)
	{
		...
		return Math.max(tpvalue, riskOfWin(fpvalue, fp, tp));
	}
	...
	//		(or neglected to attack)
        //      - if the AI piece is weak and the opponent is a bluffer
        //              (i.e. not an idiot bot)
	...
	public boolean isEffectiveBluffLoses(Piece fp, Piece tp)
	{
        ...
        // So the following check avoids bluffing if the opponent can
	...
	private int stealthValue(Piece p)
	{
		...
		// Unknowns that chase unknowns have low stealth.
                // So we assign a stealth value of a Five.

		// If an unknown piece flees from an opponent unknown piece,
		// it is an indication that the opponent piece
		// is either a high ranked piece (7-9, Spy)
                // avoiding capture
                // or an unknown low ranked piece trying to maintain stealth.
                // The high ranked piece would not have very much stealth,
		// so we assign the stealth value of a Seven, which
                // is much greater than a Five, but not as much as a Four.
                ...
                if (rank == Rank.UNKNOWN
                    ...
                    if (rank == Rank.UNKNOWN)
                        return stealthValue(p.getColor(), maybeRank);
	...
	//
	// The AI believes that if it has a copious material advantage,
	// it can risk more to defend bomb structures.  If it
	// is losing anyway, it doesn't bother.
	//
	...
	{
            int color = p.getColor();
            int value = Math.max(pieceValue(1-color, 8), isWinning(color)/3) + grid.defenderCount(color, p.getIndex())*3;
	...
	protected int distanceFactor(Piece aiPiece, TestPiece oppPiece, boolean scoutFarMove)
	{
	...
	public int winFight(Piece fp, Piece tp)
	{
		...
		// the chaser from its target.
                ...
                // is a LIKELY_SPY
                            ...
                            && isWinning(Settings.topColor) <= pieceValue(tp)/2
		...
		// A piece in the foray lane is safe from attack
		// from unknown umoved pieces.  Thus a Miner will
		// take an undefended bomb in the foray lane,
		// because the Miner is safe from attack.
		...
		// which should eliminate high value Miners.
			...
			else if (tprank.ordinal() <= lowestUnknownExpendableRank + 1) {
	...
	protected void genFleeRankandWeak()
	{
		...
		// Encourage attack on unknown pieces in the foray lane.

			if (unk.getColor() == Settings.bottomColor
                                && forayLane[Settings.topColor] == i
				&& !unk.hasMoved()) {
					if (unk.getRank() == Rank.BOMB)
						unmovedValue[i] = aiBombValue(unk);
					else if (!unk.isKnown())
						unmovedValue[i] = pieceValue(Settings.topColor, Rank.NINE);
			}
			if (unk.isKnown())
				continue;
	...
	void markWeakPieces()
	{
        ...
        // R  b?        R  b?
        // b? b?        b? b?
        // The idea is to have some backup (stacking) before attack.
        // In the both patterns, Red is safe from attack by unknowns.
        // In the second pattern, Red is safe to attack lower Blue.

        boolean forayPattern(int i)
        {
            if (forayLane[Settings.topColor] != Grid.getX(i))
                return false;

            if (i < 34)
                return false;

            Piece pp2 = getPiece(i-22);
            Piece pp1 = getPiece(i-11);
            return (pp1 != null && pp1.getColor() == Settings.topColor)
                ||  (pp2 != null && pp2.getColor() == Settings.topColor);
        }
	...
	// should be removed.  If it is lucky, the flag will be
	...
	// come in its way.
	//
	// The AI chooses the lane on the side where it has the most
	// firepower, at least a Two or Three.  It plays the odds that an
	...
	{
            return randomForay || forayPattern(i);
	}
