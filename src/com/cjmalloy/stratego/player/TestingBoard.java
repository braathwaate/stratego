...
public class TestingBoard extends Board
{
	...
	private static final int DEST_PRIORITY_DEFEND_FLAG_STAY = VALUE_NINE+1;
...
@SuppressWarnings("unchecked")
	...
	public TestingBoard(Board t)
	{
		...
		// far forward on the board as possible
            ...
            if (!hasPlan(PLANA, p.getColor(), r))
                planPiece[p.getColor()][r-1][PLANA]=p;
            else if (!hasPlan(PLANB, p.getColor(), r))
		...
		// setunmovedValues depends on neededPiece:
	...
	void adjustPieceValues()
	{
		...
		// The stealth value of the Spy is worth somewhere between
		// a Three and a Four with the opponent One still on the board.
        // Spy stealth also falls in value as ranks are removed and
        // the One falls in value.
        ...
        // erroneous if the opponent could recapture the Spy.
                ...
                valueStealth[c][9] = values[c][1] / 5;

		// A suspected spy is worth less because it is easily avoided,
        // but this depends on the remaining pieces, because in an endgame with
		// Ones on the board, the Spy can be the critical piece to win the game.

                if (c == Settings.bottomColor)
                    valueStealth[c][9] /= 2; // TBD: depends on remaining pieces
	...
	private void genValueStealth()
	{
        ...
        // The stealth value of an AI bomb is the piece value of a Scout,
        // which lies between a Three and Four stealth, so the AI will shelter
        // a bomb with Four but not a Three from attack from a weak piece.

        valueStealth[Settings.topColor][Rank.BOMB.ordinal()-1] =
            (valueStealth[Settings.topColor][Rank.THREE.ordinal()-1]
            + valueStealth[Settings.topColor][Rank.FOUR.ordinal()-1]) / 2;
	...
	void attackLanes()
	{
		...
		// as well.
				...
				if (!isInvincible(p)) {
                    if (aiPieceCount++ >= 2)
                        setPlan(getPlan(p), fleetmp[Settings.topColor], DEST_PRIORITY_LANE);
        ...
        // until a path to the flag has been identified.
			...
			if (aiInvinciblePiece != null
				...
				int plan[][] = getPlan(aiInvinciblePiece);
                if (plan != null && plan[0][0] == 0)
					setPlan(getPlan(aiInvinciblePiece), fleetmp[Settings.topColor], DEST_PRIORITY_LANE);
			}
        ...
        // is actually a piece, then it cannot pass
					...
					for (int r = oppRank; ranksNeeded <= 3 && r > 1; r--) {
						if (rankAtLarge(c, r) != 0) {
							int tmp2[] = genDestTmpGuardedRank(1-c, goal, Rank.toRank(r));
							if (!isStealthy(planPiece[c][r-1][PLANA], oppRank))
								genPlan(PLANA, false, tmp2, c, r, DEST_PRIORITY_CHASE);
							if (!isStealthy(planPiece[c][r-1][PLANB], oppRank))
								genPlan(PLANB, false, tmp2, c, r, DEST_PRIORITY_CHASE);
							ranksNeeded++;
						}
					}
				}
	...
	void setUnmovedValues()
	{
        ...
        int neededExpendableCount=0;
        int neededMinerCount=0;
		for (int i=12;i<=120;i++) {
			if (!Grid.isValid(i))
				continue;
			Piece p = getPiece(i);
	...
	protected void attackBomb(Piece bomb)
	{
        ...
        // Chase it off with an Unknown.

        if (tp != null) {
            if (isInvincibleDefender(tp))
                tp = bomb;
            chaseWithUnknown(PLANA, tp, destTmp, DEST_PRIORITY_ATTACK_FLAG);
            chaseWithUnknown(PLANB, tp, destTmp, DEST_PRIORITY_ATTACK_FLAG);
        }
        ...
        genPlan(PLANB, false, destTmp2, 1-bombColor, 8, DEST_PRIORITY_ATTACK_FLAG);
	}
	...
	protected void chaseWithExpendable(Piece p, int i, int priority)
	{
		...
		for ( int r = 1; r <= 10; r++)
			if (isExpendable(1-p.getColor(), r)) {
				...
				genNeeded(false, tmp, 1-p.getColor(), r, priority);
			}
	}
	...
	protected void chaseWithUnknown(int plan, Piece p, int tmp[], int priority)
	{
		...
		for ( int r = 1; r <= 10; r++) {
            if (unknownRankAtLarge(1-p.getColor(), r) == 0)
                continue;
			Piece a = planPiece[1-p.getColor()][r-1][plan];
			if (a != null) {
				if (a.isKnown())
					continue;
		...
		if (!found && valuableRank != 0)
				genPlan(plan, rnd.nextInt(2) != 0, tmp, 1-p.getColor(), valuableRank, priority);
	}
    ...
    Piece chaseNear(Piece p, Piece first)
    {
		...
		// Only the closest non-invincible rank is assigned
        ...
        for (int j = chasedRank - 1; j > 0; j--) {
            if (isInvincible(1-p.getColor(),j))
                continue;
            for (Piece pp : planPiece[1-p.getColor()][j-1]) {
                if (pp != null
                    && pp != first
                    && !isProtected(p, pp)
                    && !isStealthy(pp, chasedRank)
                    && Grid.steps(pp.getIndex(), i) < minsteps) {
                    found = pp;
                    minsteps = Grid.steps(found.getIndex(), i);
                }
            }
        }
		...
		// making the player Four invincible, because 1?x4 is positive).
				...
				int priority = DEST_PRIORITY_CHASE;
				if ((found.getRank() == Rank.THREE
					&& unknownNotSuspectedRankAtLarge(p.getColor(), Rank.TWO) == 0)
                    || isForayLane(1-p.getColor(), p.getIndex()))
					priority = DEST_PRIORITY_CHASE_ATTACK;

				int destTmp2[] = genDestTmpGuardedOpen(p.getColor(), i, found);
				genNeededPlan(destTmp2, 1-p.getColor(), found, priority);
        }
        return found;
    }
...
assert p.getRank() != Rank.UNKNOWN : "Unknown cannot be known or suspected " + p.isKnown() + " " + p.isSuspectedRank();
		...
		Piece opp = planPiece[1-p.getColor()][chasedRank][0];
		if (opp != null
                    ...
                    if (p.getActingRankFleeLow() != Rank.NIL
                        ...
                        for (int j = p.getActingRankFleeLow().ordinal(); j > 0; j--)
                            if (!isInvincible(1-p.getColor(),j)) {
                                ...
                                genPlan(PLANA, rnd.nextInt(2) != 0, destTmp2, 1-p.getColor(), j, DEST_PRIORITY_CHASE);
                            }
                        ...
                        for ( int j = 2; j < 10; j++)
                            if (isExpendable(1-p.getColor(), j)) {
                                int destTmp2[] = genDestTmpGuardedOpen(p.getColor(), i, Rank.toRank(j));
                                genPlan(PLANA, rnd.nextInt(2) != 0, destTmp2, 1-p.getColor(), j, DEST_PRIORITY_CHASE);
                                genPlan(PLANB, rnd.nextInt(2) != 0, destTmp2, 1-p.getColor(), j, DEST_PRIORITY_CHASE);
                            }
                        ...
                        && p.getActingRankFleeLow().ordinal() >= 5) {
                            ...
                            genPlan(PLANA, rnd.nextInt(2) != 0, destTmp2, 1-p.getColor(), p.getActingRankFleeLow().ordinal(), DEST_PRIORITY_CHASE);
                            int destTmp3[] = genDestTmpGuardedOpen(p.getColor(), i, Rank.toRank(p.getActingRankFleeLow().ordinal()+1));
                            genPlan(PLANB, rnd.nextInt(2) != 0, destTmp3, 1-p.getColor(), p.getActingRankFleeLow().ordinal()+1, DEST_PRIORITY_CHASE);
                    }
		...
		// is an important goal.
                ...
                if (isInvincibleDefender(1-p.getColor(), j)
                    ...
                    genPlan(PLANA, neededNear, destTmp2, 1-p.getColor(), j, priority);
                    genPlan(PLANB, neededNear, destTmp2, 1-p.getColor(), j, priority);
                }
	...
	private Piece getDefender(int color, int destTmp[], Rank attacker, int attackerValue, int stepsTarget, Piece pAttacker)
	{
		...
		// When the flag is no danger, and then the
		...
		// so R7 moves up, and unknown Blue moves down.
            ...
            if (pAttacker != null
                && color == Settings.topColor
                && Grid.steps(pAttacker.getIndex(), pDefender.getIndex()) <= 2) {
		...
		// use it.  Fix this in getDefender().
			...
			setDefender(pDefender, destTmp, color == Settings.topColor ? DEST_PRIORITY_DEFEND_FLAG : DEST_PRIORITY_DEFEND_FLAG_AREA);
        }
	...
	void setDefender(Piece defender, int tmp[], int priority)
	{
		...
		planPiece[color][r-1][0] = (TestPiece)defender;
	...
	private void flagBombTarget(Piece flagp)
	{
		...
		// to a lower-ranked unknown attacker.
				...
				getDefender(color, destTmp, Rank.EIGHT, pieceValue(1-color, Rank.EIGHT), stepsAttacker, pAttacker);
		...
		// area, because overreaction is a dead giveaway).
				...
				if (pProtector != null && flagp.isKnown()) {
					Piece defender = getDefender(color, destTmp, pProtector.getRank(), pieceValue(pProtector), stepsProtector, null);
					if (defender != null) {
    ...
    {
		Piece attackPiece = planPiece[1-color][attackRank.ordinal()-1][PLANA];
        if (attackPiece == null)
            attackPiece = planPiece[1-color][attackRank.ordinal()-1][PLANB];
        return attackPiece;
    }
	...
	private void setNeededRank(int plan, int color, int rank, int priority)
	{
        setNeededPiece(planPiece[color][rank-1][plan]);
	}
	...
	private void genNeeded(boolean neededNear, int [] desttmp, int color, int rank, int priority)
	{
        for (TestPiece pp : planPiece[color][rank-1]) {
            if (pp != null) {
                setPlan(neededNear, pp.plan, desttmp, color, rank, priority);
                setNeededPiece(pp);
            }
        }
	}

	private void genNeededPlan(int [] desttmp, int color, Piece p, int priority)
	{
        int rank = p.getRank().ordinal();
        for (TestPiece pp : planPiece[color][rank-1]) {
            if (p == pp)
                genNeeded(rnd.nextInt(2) != 0, desttmp, color, rank, priority);
        }
	}
	...
	private void genPlanAll(int [] desttmp, int color, int rank, int priority)
	{
        for (TestPiece pp : planPiece[color][rank-1])
            if (pp != null)
                setPlan(pp.plan, desttmp, priority);
	}
	...
	private void genPlan(int plan_name, boolean neededNear, int [] desttmp, int color, int rank, int priority)
	{
        TestPiece p = planPiece[color][rank-1][plan_name];
        if (p != null)
            setPlan(neededNear, p.plan, desttmp, color, rank, priority);
	}
	...
	protected int setProtector(Piece p, int to)
	{
        ...
        // but the opponent can eventually figure it out,
        // by tracking both pieces.   
        // The AI assumes that the opponent will believe
        // that the moved protector is the superior piece, even
        // if there is another unknown protector.
	...
	public boolean hasPlan(int plan, int color, int r)
	{
		return planPiece[color][r-1][plan] != null;
	}
	...
	public void setNeededPiece(TestPiece p)
	{
        ...
        if (isWinning(1-p.getColor()) > VALUE_THREE)
            return;
	...
	public void addMoveValue(int m, boolean adjacent)
	{
		...
		// the 8th row.  Prior to Version 13, the bias was towards
        // the suspected flag.  The problem is that this caused
        // stacking and piece bunching made them easier to pick
        // off.  The 8th row allows other pieces to go around them
        // and is close to the lane if a getaway is needed.
        ...
        // aimlessly around the board.
            ...
            if (fp.getMoves() - fp.boardPiece().getMoves() == 0) {
...
*/
		...
		// and so Blue would get a negative value from valueBluff().
					...
					if (!fp.isKnown()) {
						vm += apparentWinValue(fp, fprank, unknownScoutFarMove, tp, stealthValue(tp))
                             - stealthValue(Settings.bottomColor, tprank.ordinal());
		...
		// could be bomb
					...
					if (isPossibleBomb(tp)
						&& !tp.isKnown()
						&& !(fprank == Rank.EIGHT || tp.is(Piece.FLAG_BOMB))
						&& !wasChased) {
		...
		// so the remaining unmoved pieces are fair game?
                            ...
                            vm += Math.max(stealthValue(tp), riskOfWin(fpvalue, fp, tp, true)) - fpvalue;
                    }
	...
	public int unknownValue(int fpvalue, Piece fp, Piece tp)
	{
		...
		return Math.max(tp_stealth, riskOfWin(fpvalue, fp, tp, false));
	}
	...
	//	- if the AI piece is known or expendable
	...
	public boolean isEffectiveBluffWins(Piece fp, Piece tp, int m)
	{
		return isEffectiveBluff(fp, tp)
			&& !(blufferRisk >= 3
                && (fp.is(Piece.WEAK) || isWeakAggressive(fp)))
	...
	private int stealthValue(Piece p)
	{
        ...
        // If the AI piece has been chasing superior pieces,
        // the opponent probably has already guessed it

            if (rank.ordinal() <= 3
                && p.getActingRankChase().ordinal() <= 3)
        ...
        // but not a Three, whereas a known or weak piece would.
            ...
            return Math.max(actualStealth, stealthValue(p.getColor(), 4));
        }
	...
	int apparentRisk(Piece fp, Rank fprank, boolean unknownScoutFarMove, Piece tp)
	{
		...
		if (unknownScoutFarMove) {
            ...
            risk = Math.min(risk, weakRanks(fp.getColor())*10);
	...
	protected int distanceFactor(TestPiece aiPiece, TestPiece oppPiece, boolean scoutFarMove)
	{
		...
		if (scoutFarMove) {
            if (oppPiece.getMoves() - oppPiece.boardPiece().getMoves() != 0)
                return 2;
            else
                return 1;
        }
	...
	protected int apparentWinValue(Piece fp, Rank fprank, boolean unknownScoutFarMove, Piece tp, int v)
	{
		...
		// -- bb --
		// B2 -- --
		// Thus if a piece is chased, the risk of it
        // attacking an unknown piece is high, although normally
        // it is quite low (see wasChased).
		//
	...
	int riskOfLoss(Piece fp, Piece tp)
	{
        ...
        // b?xR1 WINS, so riskOfLoss is not called.  Here, is(Piece.LIKELY_SPY)
        // is called to be consistent with is(Piece.SAFE) in winFight().  Thus,
    ...
    String getDebugInfo()
    {
        ...
        for (int c = RED; c <= BLUE; c++) {
            ...
            for (int plan = 0; plan < 2; plan++) {
                ...
                if (p != null)
                    s += "(" + Rank.toRank(rank + 1) + "," + plan + "," + p.getIndex() + "," + isNeededPiece(p) + ")\n";
            }
            s += "\n";
